  ...
  return (
    <div className="space-y-6 md:space-y-8 font-body">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
        {statCards.map((stat) => (
          <div key={stat.name} className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100 transition-all hover:shadow-md">
            <div className="flex justify-between items-start mb-3 md:mb-4">
              <div className="p-2 md:p-3 bg-accent/10 text-accent rounded-xl">
            ...
            <p className="text-gray-400 text-[10px] md:text-xs font-semibold uppercase tracking-wider">{stat.name}</p>
            <h3 className="text-lg md:text-2xl font-bold text-primary mt-1">{stat.value}</h3>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-8">
        <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100">
          <h3 className="text-base md:text-lg font-bold text-primary mb-4 md:mb-6">Revenue Performance Overview</h3>
        ...
        <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100">
          <h3 className="text-base md:text-lg font-bold text-primary mb-4 md:mb-6">Bookings Distribution By Month</h3>
          <div className="h-60 md:h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlyStats}>
                ...
                <Tooltip 
                  ...
                  contentStyle={{borderRadius: '16px', border: 'none', boxShadow: '0 10px 25px -5px rgb(0 0 0 / 0.05)', fontSize: '12px'}}
                />
                <Bar dataKey="bookings" fill="#3B1E54" radius={[6, 6, 0, 0]} barSize={28} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-4 md:p-6 border-b border-gray-100 flex justify-between items-center">
          <h3 className="text-base md:text-lg font-bold text-primary">Recent Client Bookings</h3>
          <Link to="/admin/bookings" className="text-xs md:text-sm font-semibold text-accent hover:underline">
            View All
          </Link>
        </div>
